...
export default function StudentProgress({ onSignOut }) {
    ...
    // 2) load assignments & submissions
    useEffect(() => {
        if (!subjectId) return;
        axios.get(`/student/subjects/${subjectId}/assignments`, {
            ...
            .then(arr => {
                const pts = arr.filter(x => x);
                setData(pts);
                if (pts.length) setSelectedId(pts[0].id);
            })
            .catch(console.error);
    }, [subjectId]);
            ...
            <div className="lecturer-panels">
                ...
                <aside className="lecturer-left-panel">
                    <div className="nav-box">
                        <StudentNavBar onSignOut={onSignOut} />
                    </div>
                </aside>
